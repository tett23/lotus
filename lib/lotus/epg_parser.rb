module Lotus
  class EPGParser
    ...
    Lotus::EPGParser::EPISODE_DATA_REGEX = /(?:#|第)(\d+)(?:話|回)?.*「(.+?)」/
    Lotus::EPGParser::ONLY_EPISODE_NUMBER_REGEX = /^.*#(\d+)$/
    Lotus::EPGParser::TITLE_FIRST_EPISODE_NUMBER = /^(.+?)\s*#(\d+)\s*「(.+)」.*$/
    Lotus::EPGParser::TITLE_AFTER_EPISODE_NUMBER = /^(.+?)\s*「(.+)」\s*#(\d+).*$/
    ...
    def process_program
      # プログラムデータのパース
      if @episode_data.match(Lotus::EPGParser::EPISODE_DATA_REGEX) # #12「hogehoge」みたいな形式
        matched = @episode_data.match(Lotus::EPGParser::EPISODE_DATA_REGEX).to_a
        @episode_number = matched[1]
        @episode_name = matched[2]
      elsif @episode_data =~ Lotus::EPGParser::ONLY_EPISODE_NUMBER_REGEX # サブタイなし、話数のみ
        @episode_name = nil
        @episode_number = @episode_data.gsub(Lotus::EPGParser::ONLY_EPISODE_NUMBER_REGEX, '\1')
    ...
    def process_name
      ...
      if @episode_data.match(Lotus::EPGParser::EPISODE_DATA_REGEX) # #12「hogehoge」みたいな形式
        ...
        @episode_number = matched[1]
        @episode_name = matched[2]
      elsif @name =~ Lotus::EPGParser::TITLE_FIRST_EPISODE_NUMBER # タイトルにすべての情報が含まれている
        matched = @name.match(Lotus::EPGParser::TITLE_FIRST_EPISODE_NUMBER).to_a
        matched.delete_at(0)
        @name, @episode_number, @episode_name = matched
      elsif @name =~ Lotus::EPGParser::TITLE_AFTER_EPISODE_NUMBER # タイトルにすべての情報が含まれている
        matched = @name.match(Lotus::EPGParser::TITLE_AFTER_EPISODE_NUMBER).to_a
        matched.delete_at(0)
        @name, @episode_name, @episode_number = matched
      elsif @name =~ Lotus::EPGParser::ONLY_EPISODE_NUMBER_REGEX # タイトルに話数のみある
        @episode_number = @name.gsub(Lotus::EPGParser::ONLY_EPISODE_NUMBER_REGEX, '\1')
    ...
    def filter!
      FilterRegexp.list.each do |filter|
        case filter.target
        when :description
