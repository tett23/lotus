module Lotus
  class TS
    ...
    def add_program(program_file=nil)
      program_file ||= @original_name+'.program.txt'
      path = "#{Lotus.config[:ts_dir]}/#{program_file}"
    ...
    def add_error(error_file=nil)
      error_file ||= @original_name+'.err'
      path = "#{Lotus.config[:ts_dir]}/#{error_file}"
      return unless File.exists?(path)
